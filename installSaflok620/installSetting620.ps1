...
$cname = '[dormakaba]'
$saflokVersion = '6.2'
$hotelChain = 'Peninsula Hotel'  ### < = default for Peninsula ###
Function Write-Colr {
...
Function Logging ($state, $message) {
	...
	Switch ($state)
	{
		...
		PROCESSING  {Write-Colr -Text $part1,$part2,$part3,$part4,$part5 -Colour White,White,White,White,White}
...
If ($hotelChain -like '*Peninsula*') { 
    $configFolder = Join-Path $pluginSrcFolder 'configFiles' | Join-Path -ChildPath 'Vip'
    $pmsConfig = Join-Path $configFolder 'LENS_PMS.exe.config'
    $gatewayConfig = Join-Path $configFolder 'LENS_Gateway.exe.config'
    $webConfig = Join-Path $configFolder 'Web.config'
}
# ----------------------------------------------------------------------------------------------
# SOURCE FOLDER - Web service PMS Tester
$webServiceTester = Join-Path $pluginSrcFolder 'Web_Service_PMS_Tester'
# ----------------------------------------------------------------------------------------------
# SOURCE FOLDER - INSTALLED EXE
...
$lensGatewayConfig = 'Program Files (x86)\dormakaba\Messenger Lens\HubGatewayService\LENS_Gateway.exe.config'
If ($installOnC) {
    ...
    $lensGatewayConfigFileInst = Join-Path $installDrive $lensGatewayConfig
...
If ($confirmation -eq 'Y' -or $confirmation -eq 'YES') {
    ...
    $webInstFolder = 'C:\inetpub\wwwroot\LensPMSWebService'
    If (Test-Folder -folder $webConfig) {
        Copy-Item -Path $webConfig -Destination $webInstFolder -Force
    }
    $gatewayInstFolder = Split-Path ($instGatewayExe)
        If (Test-Folder -folder $gatewayConfig) {
        Copy-Item -Path $gatewayConfig -Destination $gatewayInstFolder -Force
    }
	# -------------------------------------------------------------------
	# INSTALL WEB SERVICE PMS TESTER
	$fileCopied = 0 
	...
	If (Assert-IsInstalled "Messenger LENS") {
		...
		[string[]]$servicesCheck = 'DeviceManagerService','KIPEncoderService','Kaba_KDS','MessengerNet_Hub Gateway Service','MessengerNet_Utility Service','VirtualEncoderService'
		...
		# ----------------------------------------------------------------
		# LENS GATEWAY CONFIG	
		If ((Assert-isInstalled  "Messenger LENS") -and (Test-Folder $lensGatewayConfigFileInst)) {
			Logging " " "==> LENS_Gateway.exe.config"
			Start-Process notepad $lensGatewayConfigFileInst -WindowStyle Minimized; Start-Sleep -S 1
		}	
		...
		[string[]]$recoveryServices = 'MNet_HMS','Kaba_KDS'
        ...
        Logging "" "+---------------------------------------------------------"
        $gatewayVer = Get-FileVersion $instGatewayExe
        $hmsVer = Get-FileVersion $instHmsExe
        $wsPmsVer = Get-FileVersion $instLensPmsExe
        $kdsVer = Get-FileVersion $instKdsExe
