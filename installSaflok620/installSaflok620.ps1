...
.NOTES
    ...
    Saflok version: v6.20, Marriott ONLY
...
If ($confirmation -eq 'Y' -or $confirmation -eq 'YES') {
	...
	Logging "" "Checking System Environment Parameter, this will take a while ..."
	If ($winOS -le 6.1) 
		{
	...
	If (($winOS -eq 6.1) -or ($winOS -gt 6.1)) 
		{
			$disabledFeatures = @()
    ...
    If ($isInstalledMesgLens -ne $True) {
      Logging "WARN" "Windows will reboot after installing Messenger Lens"
